            ...
            <div className="flex flex-col items-center w-full">
                ...
                <div className="relative mb-2 min-h-[2rem] flex items-center justify-center">
                    <AnimatePresence mode='wait'>
                        <TitleBlock key={title} title={title} />
                    </AnimatePresence>
                </div>
...
function TitleBlock({ title }: { title: string }) {
    // 1. SCRAMBLE LOGIC
    // We trigger the hook whenever the 'title' prop changes key (because of parent key)
    const scrambleTitle = useScrambleText(title, 800);
    ...
    }, []); // Only run on mount, as the key change re-mounts this component
